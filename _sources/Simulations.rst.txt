Simulations of Photo-Physical Properties
==============================================
...
#########################################
The :ref:`absorption spectrum` is calculated based on these three :ref:`engines`. 

* :doc:`NWChem_Calculation`,
* :doc:`GPAW_Calculation` and 
* :doc:`Octopus_Calculation`. 
...
can be mimicked in the design of artificial organic devices.
One standard method to interpret experimental data of excitation energy transfer between a donor (D) and an acceptor (A)
...
acceptor molecules (for example, see :ref:`Fig.2 <Fig.2>`). One of the central assumptions in FRET is that the coupling between D and A
...
.. figure:: ./Image_laser/eet.png
    ...
    *Fig.2: Schematic of the non-radiative resonance energy transfer 
    between the excited state donor and the ground state acceptor molecules.*


Recently, TDDFT_ has been applied to Forster type excitation energy transfer questions in the Casida type linear response as
well as in RT-TDDFT_. Nowadays, RT-TDDFT is finding an increasing attention due to its accuracy and favorable scaling, which
allows one to apply it large systems. The RT-TDDFT_ solutions, particularly those based on the linear combination of atomic
orbitals (LCAO) as basis sets, can provide insights into the electronically excited state dynamics for the large complex materials.
To study the EET using RT-TDDFT_, it is convenient to start from an initially localized excitation of the molecular aggregates (for
example, see :ref:`Fig.3 <maeet>`). However, such a feature is presently unavailable in most of the LCAO-basis RT-TDDFT codes. Motivated
by this need, we have developed the LASER MASKING tool on the LCAO basis employing the GPAW code. This way, we can
excite only the desired molecule in molecular or nanoparticle assemblies.

.. _maeet:

.. figure:: ./Image_laser/molecular_aggregate_eet.png
    :width: 400px
    :align: center
    :height: 250px
    :alt: alternate text
    ...
    **Fig.3: Schematic of the locally exciting a molecule in the molecular aggregate** 
                     

The Hamiltonian of the system in the presence of laser masking is given as

.. math::
    \mathbf H(\mathbf t) = - \frac{\nabla^2}{2} + \mathbf M(\mathbf r)\mathbf V_\text{ext}({\mathbf r}, t) + \mathbf V(\mathbf r)
    \label{Eq:H(t)}
    \tag{14}


where `\mathbf V_{ext}` is the externally applied time-dependent potential and V is the internal potential energy due to electrons and ions.
The masking function M(r) in Eq. :math:`\ref{Eq:H(t)}` is defined as

.. math::
    \begin{eqnarray}
    M(\mathbf r) = 
    \begin{cases}
      1, & \text{Unmasked region} \\        
      0, & \text{Masked region}
    \end{cases}
    \label{Eq:Mr}
    \tag{15}
...
The factor V in Eq. :math:`\ref{Eq:K^ET}` is the electronic coupling matrix element and is given as


.. math::
    \mathbf V = \langle D\mathbf A^{*}|\hat{\mathbf V}^{C}|D^*\mathbf A \rangle
    \label{Eq:V}
    \tag{17}

where the Coulomb interaction  `\hat V^C` mediates between the initial and final wave functions. F¨orster theory is based on the assumption
that the wave function of the total system can be separated into D and A parts due to negligible electronic coupling between
D and A. Initially, the acceptor is in its ground state denoted by `|\mathbf A \rangle` and the donor is in an excited state `|D^⋆\rangle`.

The coupling matrix element in Eq. :math:`\ref{Eq:V}` within the FRET theory is obtained as

.. math::
    V^\text{FRET} = \kappa^{DA} \frac{{\lvert \mu^A\rvert}{\lvert \mu^D \rvert}}{{\lvert {\mathbf R} \rvert}^3}
    \label{Eq:V^F}
    \tag{18}


where κDA is the orientation factor and μA and μD are the transition dipole moments of acceptor and donor molecules, respectively
(for more details, refer to [8]_ ).


Together with Eq. :math:`\ref{Eq:K^ET}` the above equation yields the rate of the Forster type excitation energy transfer.


**Evaluating the Coupling Matrix element from a real-time propagation**
***************************************************************************

The most important question to ask is how to extract the coupling constant matrix element as given in Eq. :math:`\ref{Eq:V}`. 
In principle one could calculate the the initial and final states and evaluate Eq. :math:`\ref{Eq:V}` directly. As we know, the many-body states 
are hard to compute from first principles and there is no solid reason to identify the KS Slater determinant with the true wave function.
However, there exists an alternative approach to calculate them in terms of the Davydov splitting. In the case two same molecules, the
...
This observation can be exploited in TDDFT_ by calculating the spectrum and extracting the coupling matrix element from the energy splitting.
In the real-time approach to TDDFT spectra are usually determined by initially exciting
the system with a momentum boost, propagating the time-dependent KS equations using the dipole moment as observable, and calculating the 
dipole spectrum via a Fourier transformation. As the energy splitting is typically much smaller than the excitation energies themselves, 
high resolution and thus comparably long propagation times are needed to resolve the splitting.
...
Apart from the beat the dipole moment of the system oscillates only at frequencies close to the laser frequency that was used for the excitation. 
Hence the beat frequency can be determined easily from the `D` and `A` dipole moments. The essential observation now is that for doing so, the 
dipole moment time evolution only has to be recorded long enough to capture one half cycle of the beat. This is a much shorter time than the time 
needed to obtain an accurate spectrum from the Fourier transformation. Thus, the coupling strength can be determined with moderate simulation times.
...
.. [8] D. Hofmann, T. Korzdorfer, and S. Kummel, Energy transfer and forster’s dipole coupling approximation investigated in a real-time
       kohn-sham scheme, Phys. Rev. A 82, 012509 (2010).
